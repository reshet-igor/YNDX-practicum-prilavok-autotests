...
#Тест №10а: Создание набора без передачи всех необходимых данных
# Я не могу написать одну функцию, которая покрывала бы вообще все проверки
# Или придется в другие тесты прописывать отдельное создание индивидуального kit_body, что явно больше чем написать один тест
# Нигде в задании не стоит требование по обязательности применения одной функции для всех негативных тестов, есть лишь только слово МОЖНО
# Но если вам так хочется, то вот решение как из тренажера, с созданием отдельной функции для тестов без какой-то части тела запроса
def test_10а_create_kit_no_name_failed():
    kit_body = data.kit_body.copy() #Копирование текущего тела набора
    kit_body.pop("name") # Удаление параметра name из запроса
    negative_assert_no_data(kit_body) # Проверка полученного ответа без модификации
